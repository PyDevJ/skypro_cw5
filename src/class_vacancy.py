...
class Vacancy:
    ...
    def __str__(self):
        ...
        Тип - {self.type}
        Дата публикации - {datetime.datetime.fromtimestamp(self.data_published).strftime('%Y-%m-%d %H:%M:%S')}
    ...
    def create_vacancy_from_hh(cls, vacancy_info_hh: dict):
        """
        Метод класса преобразует информацию о вакансии с платформы hh.ru и создает объект Vacancy.
        """
        result = {
    ...
    def check_for_availability(vacancy_information: dict, param1: str, param2: str = None, param3: str = None):
        """
        Статический метод используется при создании объекта Vacancy. Проверка параметра на доступность.
        """
        try:
            if param3 is None:
                if param2 is None:
